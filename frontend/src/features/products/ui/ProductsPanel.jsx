    ...
    <>
      ...
      <div className="card" style={{ padding: "12px 20px" }}>
        <form className="search-bar" onSubmit={handleSearch} style={{ marginBottom: 0 }}>
          ...
          {search && <button type="button" className="btn btn-ghost btn-sm" onClick={() => { setSearch(""); applySearch(""); }}>Clear</button>}
                    ...
                    <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                      {restockId === p.id ? (
                        <>
                          ...
                          <button className="btn btn-success btn-sm" onClick={async () => { await restock(p.id, restockQty); setRestockId(null); }}>✔ Apply</button>
